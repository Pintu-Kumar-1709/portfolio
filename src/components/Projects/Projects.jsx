...
function Projects() {
  ...
  const categories = useMemo(
    () => getProjectCategories(Array.isArray(projects) ? projects : []),
    [projects],
  );
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="space-y-6">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-neutral-800/30 rounded-2xl h-48 animate-pulse"
              />
            ))}
          </div>
        </div>
              ...
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                  filter === category.value
                    ? "bg-linear-to-r from-amber-500 to-amber-600 text-neutral-900 shadow-lg shadow-amber-500/20"
...
function ProjectCard({ project, index }) {
  ...
  return (
    ...
    >
      <div
        className={`relative bg-neutral-900/60 backdrop-blur-sm rounded-2xl border transition-all duration-500 overflow-hidden ${
          project.featured
      ...
      >
        ...
        {project.featured && (
          <div className="absolute top-4 right-4 z-10">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-linear-to-r from-amber-500 to-amber-600 text-neutral-900 text-xs font-medium">
        ...
        <div className="flex flex-col md:flex-row">
          ...
          <div className="md:w-72 lg:w-80 shrink-0">
            <div className="relative h-48 md:h-full overflow-hidden bg-neutral-800/50">
              ...
              {/* Overlay gradient */}
              <div className="absolute inset-0 bg-linear-to-t from-neutral-900/80 via-transparent to-transparent md:bg-linear-to-r" />
            </div>
          </div>
          ...
          <div className="flex-1 p-6 md:p-8 flex flex-col">
            ...
            {/* Description */}
            <p className="text-neutral-300 text-sm md:text-base leading-relaxed mb-5 grow">
              {project.description}
            </p>
            ...
            <div className="flex items-center gap-3">
              ...
              {project.demoUrl && project.demoUrl !== project.githubUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-linear-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-neutral-900 text-sm font-medium transition-all duration-300 shadow-lg shadow-amber-500/15 hover:shadow-amber-500/25"
                >
