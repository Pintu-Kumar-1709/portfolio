  ...
  return (
    <div className={`animate-pulse ${className}`}>
      {Array.from({ length: lines }).map((_, index) => (
        ...
        >
          <div className="absolute inset-0 bg-linear-to-r from-transparent via-neutral-600/20 to-transparent animate-shimmer"></div>
        </div>
      ))}
    </div>
  );
...
export const CardSkeleton = memo(({ className = "" }) => {
  return (
    <div
      className={`glass-effect rounded-2xl overflow-hidden border border-neutral-700/50 animate-pulse ${className}`}
    >
      {/* Image placeholder with shimmer */}
      <div className="bg-linear-to-br from-neutral-700/50 to-neutral-800/50 h-48 mb-4 relative overflow-hidden">
        <div className="absolute inset-0 bg-linear-to-r from-transparent via-neutral-600/20 to-transparent animate-shimmer"></div>
      </div>
